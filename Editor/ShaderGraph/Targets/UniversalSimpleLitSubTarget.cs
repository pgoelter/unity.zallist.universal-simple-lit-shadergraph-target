    ...
    sealed class UniversalSimpleLitSubTarget : UniversalSubTarget
    {
        ...
        public override void ProcessPreviewMaterial(Material material)
        {
            if (target.allowMaterialOverride)
            {
                ...
                if (m_ReceiveFog) material.EnableKeyword("_RECEIVE_FOG");
                else material.DisableKeyword("_RECEIVE_FOG");
        ...
        public override void CollectShaderProperties(PropertyCollector collector, GenerationMode generationMode)
        {
            if (target.allowMaterialOverride)
            {
                ...
                if (target.surfaceType == SurfaceType.Opaque) collector.AddFloatProperty(Property.DstBlendAlpha, 0.0f);
                else collector.AddFloatProperty(Property.DstBlendAlpha, 10.0f);
        ...
        internal override void OnAfterParentTargetDeserialized()
        {
            ...
            if (this.sgVersion < latestVersion)
            {
                if (this.sgVersion < 1)
                {
                    ...
                    else blendModePreserveSpecular = false;
                }
...
#elif UNITY_2022_2_OR_NEWER
                result.passes.Add(PassVariant(CorePragmas.SceneSelection(target), CorePragmas.DefaultSM45));
#else
        ...
        static class SimpleLitPasses
        {
            ...
            {
                if (target.allowMaterialOverride) pass.keywords.Add(SimpleLitDefines.SpecularColor);
                else if (specularHighlights) pass.defines.Add(SimpleLitDefines.SpecularColor, 1);
            }

            static void AddReceiveShadowsControlToPass(ref PassDescriptor pass, UniversalTarget target, bool receiveShadows)
            {
                if (target.allowMaterialOverride) pass.keywords.Add(SimpleLitKeywords.ReceiveShadowsOff);
                else if (!receiveShadows) pass.defines.Add(SimpleLitKeywords.ReceiveShadowsOff, 1);
            }
...
#if UNITY_2022_2_OR_NEWER
            public static PassDescriptor Forward(UniversalTarget target, bool blendModePreserveSpecular, bool specularHighlights, PragmaCollection pragmas, KeywordCollection keywords)
#elif UNITY_2022_1_OR_NEWER
...
#endif
            {
                var result = new PassDescriptor
                {
                    ...
                    renderStates = CoreRenderStates.UberSwitchedRenderState(target),
#endif
                    // --- FIX: Unity 6 (URP 17+) kompatibles Pragma ---
            ...
            public static PassDescriptor Meta(UniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                   displayName = "Meta", referenceName = "SHADERPASS_META", lightMode = "Meta",
                   passTemplatePath = UniversalTarget.kUberTemplatePath, sharedTemplateDirectories = UniversalTarget.kSharedTemplateDirectories,
                   validVertexBlocks = CoreBlockMasks.Vertex, validPixelBlocks = SimpleLitBlockMasks.FragmentMeta,
                   structs = CoreStructCollections.Default, requiredFields = SimpleLitRequiredFields.Meta,
                   fieldDependencies = CoreFieldDependencies.Default, renderStates = CoreRenderStates.Meta,
                   pragmas = CorePragmas.Default, defines = new DefineCollection() { CoreDefines.UseFragmentFog },
                   keywords = new KeywordCollection() { CoreKeywordDescriptors.EditorVisualization },
                   includes = SimpleLitIncludes.Meta, customInterpolators = CoreCustomInterpDescriptors.Common
                };
            ...
            public static PassDescriptor _2D(UniversalTarget target)
            {
                 var result = new PassDescriptor()
                {
                    referenceName = "SHADERPASS_2D", lightMode = "Universal2D",
                    passTemplatePath = UniversalTarget.kUberTemplatePath, sharedTemplateDirectories = UniversalTarget.kSharedTemplateDirectories,
                    validVertexBlocks = CoreBlockMasks.Vertex, validPixelBlocks = CoreBlockMasks.FragmentColorAlpha,
                    structs = CoreStructCollections.Default, fieldDependencies = CoreFieldDependencies.Default,
                    renderStates = CoreRenderStates.UberSwitchedRenderState(target),
                    pragmas = CorePragmas.Instanced, defines = new DefineCollection(),
                    keywords = new KeywordCollection(), includes = SimpleLitIncludes._2D,
                    customInterpolators = CoreCustomInterpDescriptors.Common
                };
                CorePasses.AddAlphaClipControlToPass(ref result, target);
                return result;
            ...
            public static PassDescriptor DepthNormal(UniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                    displayName = "DepthNormals", referenceName = "SHADERPASS_DEPTHNORMALS", lightMode = "DepthNormals",
                    useInPreview = false, passTemplatePath = UniversalTarget.kUberTemplatePath,
                    sharedTemplateDirectories = UniversalTarget.kSharedTemplateDirectories,
                    validVertexBlocks = CoreBlockMasks.Vertex, validPixelBlocks = CoreBlockMasks.FragmentDepthNormals,
                    structs = CoreStructCollections.Default, requiredFields = CoreRequiredFields.DepthNormals,
                    fieldDependencies = CoreFieldDependencies.Default, renderStates = CoreRenderStates.DepthNormalsOnly(target),
                    pragmas = CorePragmas.Instanced, defines = new DefineCollection(),
#if UNITY_2022_2_15_OR_NEWER
                    ...
                    keywords = new KeywordCollection(),
#endif
                    includes = CoreIncludes.DepthNormalsOnly, customInterpolators = CoreCustomInterpDescriptors.Common
                };
                CorePasses.AddAlphaClipControlToPass(ref result, target);
#if UNITY_2022_2_OR_NEWER
                CorePasses.AddLODCrossFadeControlToPass(ref result, target);
        ...
        static class SimpleLitBlockMasks
        {
            ...
            {
                BlockFields.SurfaceDescription.BaseColor, BlockFields.SurfaceDescription.NormalOS, BlockFields.SurfaceDescription.NormalTS,
                BlockFields.SurfaceDescription.NormalWS, BlockFields.SurfaceDescription.Emission, BlockFields.SurfaceDescription.Specular,
                BlockFields.SurfaceDescription.Smoothness, BlockFields.SurfaceDescription.Alpha, BlockFields.SurfaceDescription.AlphaClipThreshold,
            };
        ...
        static class SimpleLitRequiredFields
        {
            ...
            {
                StructFields.Attributes.uv1, StructFields.Attributes.uv2, StructFields.Varyings.positionWS,
                StructFields.Varyings.normalWS, StructFields.Varyings.tangentWS, UniversalStructFields.Varyings.staticLightmapUV,
                UniversalStructFields.Varyings.dynamicLightmapUV, UniversalStructFields.Varyings.sh,
                UniversalStructFields.Varyings.fogFactorAndVertexLight, UniversalStructFields.Varyings.shadowCoord,             
            };
            ...
            {
                StructFields.Attributes.positionOS, StructFields.Attributes.normalOS, StructFields.Attributes.uv0,                            
                StructFields.Attributes.uv1, StructFields.Attributes.uv2, StructFields.Attributes.instanceID,                     
                StructFields.Varyings.positionCS, StructFields.Varyings.texCoord0, StructFields.Varyings.texCoord1, StructFields.Varyings.texCoord2,                        
            };
        }
        #endregion
        ...
        static class SimpleLitDefines
        {
            ...
            {
                displayName = "Specular Setup", referenceName = "_SPECULAR_SETUP", type = KeywordType.Boolean,
                definition = KeywordDefinition.ShaderFeature, scope = KeywordScope.Local, stages = KeywordShaderStage.Fragment
            };

            public static readonly KeywordDescriptor SpecularColor = new KeywordDescriptor()
            {
                displayName = "Specular Color", referenceName = SimpleLitProperty.SpecularColorKeyword, type = KeywordType.Boolean,
                definition = KeywordDefinition.ShaderFeature, scope = KeywordScope.Local, stages = KeywordShaderStage.Fragment
            };
        }
        #endregion
        ...
        static class SimpleLitKeywords
        {
            ...
            {
                displayName = "Receive Shadows Off", referenceName = ShaderKeywordStrings._RECEIVE_SHADOWS_OFF,
                type = KeywordType.Boolean, definition = KeywordDefinition.ShaderFeature, scope = KeywordScope.Local,
            };

#if !UNITY_2022_2_OR_NEWER
            public static readonly KeywordDescriptor ScreenSpaceAmbientOcclusion = new KeywordDescriptor()
            {
                displayName = "Screen Space Ambient Occlusion", referenceName = "_SCREEN_SPACE_OCCLUSION",
...
#endif
                { CoreKeywordDescriptors.StaticLightmap }, { CoreKeywordDescriptors.DynamicLightmap },
                { CoreKeywordDescriptors.DirectionalLightmapCombined }, { CoreKeywordDescriptors.MainLightShadows },
                { CoreKeywordDescriptors.AdditionalLights }, { CoreKeywordDescriptors.AdditionalLightShadows },
                { CoreKeywordDescriptors.ReflectionProbeBlending }, { CoreKeywordDescriptors.ReflectionProbeBoxProjection },
                { CoreKeywordDescriptors.ShadowsSoft }, { CoreKeywordDescriptors.LightmapShadowMixing },
                { CoreKeywordDescriptors.ShadowsShadowmask }, { CoreKeywordDescriptors.DBuffer },
                { CoreKeywordDescriptors.LightLayers }, { CoreKeywordDescriptors.DebugDisplay }, { CoreKeywordDescriptors.LightCookies },
...
#if !UNITY_2022_2_15_OR_NEWER && UNITY_2022_2_OR_NEWER
            public static readonly KeywordCollection DOTSForward = new KeywordCollection { { Forward }, { CoreKeywordDescriptors.WriteRenderingLayers }, };
#endif
            ...
            public static readonly KeywordCollection GBuffer = new KeywordCollection
            {
                ...
                { CoreKeywordDescriptors.ReflectionProbeBlending }, { CoreKeywordDescriptors.ReflectionProbeBoxProjection },
                { CoreKeywordDescriptors.ShadowsSoft }, { CoreKeywordDescriptors.LightmapShadowMixing },
                { CoreKeywordDescriptors.MixedLightingSubtractive }, { CoreKeywordDescriptors.DBuffer }, { CoreKeywordDescriptors.GBufferNormalsOct },
#if UNITY_2022_2_15_OR_NEWER
                ...
                { CoreKeywordDescriptors.LightLayers },
#endif
                { CoreKeywordDescriptors.RenderPassEnabled }, { CoreKeywordDescriptors.DebugDisplay },
            };
        }
        #endregion
        ...
        static class SimpleLitIncludes
        {
            ...
            {
#if UNITY_2022_2_15_OR_NEWER
                { CoreIncludes.DOTSPregraph }, { CoreIncludes.WriteRenderLayersPregraph },
#endif
                { CoreIncludes.CorePregraph }, { kShadows, IncludeLocation.Pregraph },
                { CoreIncludes.ShaderGraphPregraph }, { CoreIncludes.DBufferPregraph },
                { CoreIncludes.CorePostgraph }, { kForwardPass, IncludeLocation.Postgraph },
            };
            ...
            public static readonly IncludeCollection GBuffer = new IncludeCollection
            {
                ...
                { CoreIncludes.CorePregraph }, { kShadows, IncludeLocation.Pregraph },
                { CoreIncludes.ShaderGraphPregraph }, { CoreIncludes.DBufferPregraph },
                { CoreIncludes.CorePostgraph }, { kGBuffer, IncludeLocation.Postgraph },
                { kSimpleLitGBufferPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection Meta = new IncludeCollection
            {
                { CoreIncludes.CorePregraph }, { CoreIncludes.ShaderGraphPregraph }, { kMetaInput, IncludeLocation.Pregraph },
                { CoreIncludes.CorePostgraph }, { kLightingMetaPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection _2D = new IncludeCollection
            {
                { CoreIncludes.CorePregraph }, { CoreIncludes.ShaderGraphPregraph },
                { CoreIncludes.CorePostgraph }, { k2DPass, IncludeLocation.Postgraph },
            };
        }
        #endregion
    }
